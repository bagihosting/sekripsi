...
export async function getPricingPlans(): Promise<PricingPlan[]> {
  try {
    const plansCollection = collection(db, 'pricingPlans');
    const q = query(plansCollection, orderBy('name')); // Assuming you want to order by name, adjust if needed
    ...
    // Simple sort to ensure free, pro, team order if names don't sort correctly
    ...
    // Return default plans as a fallback
    return defaultPlans;
  }
}
