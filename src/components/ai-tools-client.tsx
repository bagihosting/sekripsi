...
import { ArrowRight, Sparkles, Star, Wand } from 'lucide-react';
...
export default function AiToolsClient({ allTools, initialUserProfile }: AiToolsClientProps) {
  const { userProfile, loading } = useAuth();
  const [toolGroups, setToolGroups] = useState<AiToolGroup[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  // Determine the effective user profile, prioritizing the client-side one once it loads.
  const effectiveUserProfile = loading ? initialUserProfile : userProfile;

  useEffect(() => {
    // This effect runs on the client to filter tools based on the latest user profile
    if (loading) return; // Wait for client-side auth state to be resolved
  ...
  }, [userProfile, loading, allTools]);

  if (isLoading) {
    // Skeleton shown during the initial client-side render or while auth is resolving
    return (
